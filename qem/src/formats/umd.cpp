...
{
    // workaround for Symbian global static bug, test on Nokia 603
...
struct UmdParseData
{
    ...
    QMap<quint32, UMD::ChunkType> dataOwners;    // owner of data chunk
...
{
    if (0 == in.device()) {
        FormatsUtility::debug("No device set for QDataStream", error);
        return 0;
    }
    ...
    if (! isUMD(in)) {
        FormatsUtility::debug("Invalid UMD file: magic number", error);
        return 0;
    }
    ...
    while (true) {
        ...
        default:
        {
            FormatsUtility::debug("Bad UMD file: unknown separator", error);
            goto ERROR;
        }
        }
    }
...
bool UMD::makeUmd(const Book &book, QIODevice &device, const QVariantMap &args, QString *error)
{
    ...
    FormatsUtility::debug("UMD maker is under development", error);
    return false;
}
...
{
    qint32 index = m_offset / UMD::BUFFER_SIZE;
    qint32 start = m_offset % UMD::BUFFER_SIZE;
    qint32 length = -start;
    QByteArray data;
    do {
...
static bool readChunk(QDataStream &in, UmdParseData &umdData)
{
    ...
    UMD::ChunkType chunkType = UMD::ChunkType(chunkId);
    ...
    case UMD::CdsKey:
    {
        ...
        if (in.readRawData(bytes, length) != length) {
            delete bytes;
            FormatsUtility::debug("Bad UMD file: read CDS key", umdData.error);
            return false;
        }
    ...
    case UMD::LicenseKey:
    {
        ...
        if (in.readRawData(bytes, length) != length) {
            delete bytes;
            FormatsUtility::debug("Bad UMD file: read license key", umdData.error);
            return false;
        }
        QByteArray key(bytes);
        delete bytes;
    ...
    case UMD::End:
    {
        ...
        if (pos != in.device()->pos()) {
            FormatsUtility::debug("Bad UMD file: end position", umdData.error);
            return false;
        }
    }
        break;
...
static bool readData(QDataStream &in, UmdParseData &umdData)
{
    quint32 check;
    in >> check;
    UMD::ChunkType chunkType = umdData.dataOwners.value(check, UMD::Invalid);
...
static void readBookCover(QDataStream &in, UmdParseData &umdData)
{
    ...
    FileObject *cover = FileFactory::getFileObject(QString("cover.%1").arg(
